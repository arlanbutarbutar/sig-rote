      ...
      <script>
        ...
        ?>
            L.marker([<?= $row_w['latitude'] ?>, <?= $row_w['longitude'] ?>], {
              icon: myIcon
            }).bindPopup("<div><img src='../assets/images/wisata/<?= $row_w['image'] ?>' style='width: 100%;' alt=''><h4 style='margin-top: 5px;'><a><?= $row_w['nama_wisata'] ?></a></h4><p style='margin-top: -5px;'><?= $row_w['deskripsi'] ?></p><small style='letter-spacing: 0;'><?= $row_w['alamat'] ?></small></div>").addTo(map);
        <?php }
        } ?>
      </script>
    </div>
